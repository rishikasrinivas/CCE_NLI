...
class L0Module(Module):
    ...
    def initialize_structured_head(self, add_prunable_model_size=True):
        if self.learned_zs and 'head_z' in self.learned_zs:
            self.head_loga = Parameter(self.learned_zs['head_z'].reshape(self.num_hidden_layers, self.num_attention_heads))
    ...
    def lagrangian_regularization(self, pruned_steps):
        ...
        expected_size = self.get_num_parameters_for_mlp()
    ...
    def calculate_model_size_BOWMAN(self, zs):
        ...
        results["mlp_input_2048"] = remaining_mlp_inp
    ...
    def calculate_model_size(self,zs):
        if self.model_name=='bowman':
            return self.calculate_model_size_BOWMAN(zs)
