...
def build_masks(activations, num_clusters, cluster_num):
    activations=torch.from_numpy(activations.numpy().reshape(activations.shape[0]*activations.shape[1], 1))
    activation_ranges = create_clusters(activations,num_clusters)
    activations=torch.from_numpy(activations.numpy().reshape(10000,1024))
    print("activations ranges", activation_ranges)
    mask=build_act_mask(activations,activation_ranges, cluster_num)
    print(mask.shape)
    return mask.numpy()
