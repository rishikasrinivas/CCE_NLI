...
def get_inputs_bert(model, embedder, dataloader, dtype, device):
    ...
    if "BERTWANDA_INPS.pth" in os.listdir("code/wanda/cache"):
        inps = torch.load("code/wanda/cache/BERTWANDA_INPS.pth")
        attention_mask = torch.load("code/wanda/cache/BERTWANDA_attention_mask.pth")
        outs = torch.zeros_like(inps)
        print("Retreieve bert from cachce")
    ...
    torch.save(inps, "code/wanda/cache/BERTWANDA_INPS.pth")
    torch.save(attention_mask, "code/wanda/cache/BERTWANDA_attention_mask.pth")
    outs = torch.zeros_like(inps)
...
def get_inputs_mlp(model, embedder, args, dataloader, dtype, device):
    if "MLP_BERT.pth" in os.listdir("code/wanda/cache"):
        print("Retreieve from cachce")
        return torch.load("code/wanda/cache/MLP_BERT.pth"), None, None
    ...
    attention_mask = None
    position_ids = None
    torch.save(inps, "code/wanda/MLP_BERT.pth")
